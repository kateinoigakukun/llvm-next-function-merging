...
TEST_F(MSAFunctionMergerAlignmentTest, HyFMBasic3) {
  LLVMContext Ctx;
  SMDiagnostic Error;
  auto M = parseAssemblyString(R"(
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
...
; Function Attrs: noinline optsize uwtable
define i32 @foo(i8* nocapture %theFile, i32 %toRead, i8* nocapture %toFill, %"class.xalanc_1_8::XalanNode"* %manager) local_unnamed_addr #0 align 2 personality i8* bitcast (i32 (...)* @__gxx_personality_v0 to i8*) {
entry:
  %conv = zext i32 %toRead to i64
  %0 = bitcast i8* %theFile to %struct._IO_FILE*
  %call = tail call i64 @fread(i8* %toFill, i64 1, i64 %conv, %struct._IO_FILE* %0) #3
...
; Function Attrs: noinline optsize uwtable
define i32 @bar(i8* nocapture %theFile, i32 %toRead, i8* nocapture %toFill, %"class.xalanc_1_8::XalanNode"* %manager) local_unnamed_addr #0 align 2 personality i8* bitcast (i32 (...)* @__gxx_personality_v0 to i8*) {
entry:
  %conv = zext i32 %toRead to i64
  %0 = bitcast i8* %theFile to %struct._IO_FILE*
  %call = tail call i64 @fread(i8* %toFill, i64 1, i64 %conv, %struct._IO_FILE* %0) #3
...
; Function Attrs: noinline optsize uwtable
define i32 @baz(i8* nocapture %theFile, i32 %toRead, i8* nocapture %toFill, %"class.xalanc_1_8::XalanNode"* %manager) local_unnamed_addr #0 align 2 personality i8* bitcast (i32 (...)* @__gxx_personality_v0 to i8*) {
entry:
  %conv = zext i32 %toRead to i64
  %0 = bitcast i8* %theFile to %struct._IO_FILE*
  %call = tail call i64 @fread(i8* %toFill, i64 1, i64 %conv, %struct._IO_FILE* %0) #3
