...
FunctionMerger::merge(Function *F1, Function *F2, std::string Name,
  ...
  if (EnableHyFMNW) {
    if (EnableCodeSharing) {
      ...
      Aligner = std::make_unique<HyFMNWAligner>(HyFMProfitability);
    }
  } else if (EnableHyFMPA) {
    Aligner = std::make_unique<HyFMPAAligner>(HyFMProfitability);
  } else { // default SALSSA
    if (EnableCodeSharing) {
      Aligner = std::make_unique<MSAAlignerAdapter>(
    ...
    while (!Rank.empty()) {
      ...
      if (Result.getMergedFunction() != nullptr) {
        ...
        if (!match.Valid) {
          ...
          size_t MergedSize =
              FSE.estimate({Result.getMergedFunction()}, {F1, F2},
          ...
          size_t SizeF1F2 = FSE.estimate({F1, F2}, {Result.getMergedFunction()},
                                         Options.SizeEstimationMethod);
      ...
      if (Verbose) {
        errs() << F1Name << " + " << F2Name << " <= " << Name
               ...
               << " <= " << match.MergedSize
               << " IRSizes: " << match.OtherMagnitude << " + "
               << match.Magnitude << " Profitable: " << match.Profitable
        ...
        errs() << " TotalTime: "
               ...
               << " UpdateTime: "
               << (time_update_end - time_update_start) / 1us;
#endif
        errs() << "\n";
      }
