; RUN: %opt -S -debug-only=func-merging --passes="func-merging" --pass-remarks-output=- --pass-remarks-filter=func-merging --func-merging-whole-program=true --multiple-func-merging-coalescing=false --hyfm-profitability=true --func-merging-hyfm-nw --func-merging-disable-post-opt --func-merging-allow-unprofitable  -o %t.hyfm.ll %s 2> %t.hyfm.log | FileCheck %s --check-prefix=CHECK-HYFM
; RUN: %opt -S -debug-only=msa-hyfm     --passes="multiple-func-merging" --pass-remarks-output=- --pass-remarks-filter=multiple-func-merging --multiple-func-merging-whole-program=true --multiple-func-merging-coalescing=false --multiple-func-merging-hyfm-nw --multiple-func-merging-hyfm-profitability --multiple-func-merging-disable-post-opt --multiple-func-merging-allow-unprofitable --multiple-func-merging-identical-type=true -o %t.mfm-hyfm.ll %s 2> %t.mfm-hyfm.log | FileCheck %s --check-prefix=CHECK-MFM
; CHECK-HYFM:      --- !Passed
